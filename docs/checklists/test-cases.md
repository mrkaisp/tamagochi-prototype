...
### 1-4 メイン→設定（左）/ステータス（右）
- 前提: メイン画面
- 操作: 左（1）/右（3）
- 期待: 左で`設定画面`、右で`ステータス画面`へ
...
### 1-6 メイン→各モード→メイン復帰
- 前提: メイン画面
- 操作: モード選択（06〜08 のいずれか）実行後に終了
- 期待: 実行後は `メイン画面` に戻る

### 1-7 自動イベント遷移（花言葉/死亡）
- 前提: メイン画面で時間経過
- 入力: 成長タイマー到達
- 期待: `花言葉選択` へ自動遷移、回答完了でメイン復帰
- 入力: 栄養=0 または寿命到達
- 期待: `死亡/ニューゲーム` へ自動遷移、決定で`タイトル`へ
