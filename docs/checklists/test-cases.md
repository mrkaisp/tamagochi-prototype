...
### 2-1 解像度制約
- 前提: 全画面/全UI
- 検証: UI要素・キャラ・テキストの描画座標が 0..239 の範囲内
- 期待: 逸脱なし（960×960 物理拡大は論理解像度に影響しない）
...
- **テスト用オプション**: `config.game.nutrition_limit_disabled = True` に設定すると制限を無効化可能
