## 共通/前提
- [ ] 論理解像度 240×240 に収まる座標系を定義している（描画は 0..239 にクランプ）
- [ ] 物理解像度拡大（例: 960×960）が論理解像度計算に影響しない
...
## 2. UI/解像度・入出力
- [ ] 全UI/テキスト/キャラ描画の座標が 0..239 範囲内でガードされる
...
- [ ] **陰/陽傾向（light_tendency_yin）の決定：フェーズ1（種→芽）の成長時に、その時点での光レベルが50未満かどうかで一度だけ決定される（成長後は変更されない）**
- [ ] **光OFF処理：光レベルを減少させる処理が実装されている（現在は未実装の可能性）**
- [ ] **光ON/OFF処理：stats_observable.value が更新され、状態変更が通知される**
- [ ] **雑草除去：雑草数が0の時は「雑草はありません」メッセージを表示し、環境レベルは上昇しない**
- [ ] **害虫駆除：害虫数が0の時は「害虫はいません」メッセージを表示し、環境レベルは上昇しない**
